...
const getDateRange = (range, customStart, customEnd) => {
  ...
  if (range === "week") {
    ...
    const lastWeekStart = today - (dayOfWeek - 1 + 7) * DAY;
    const lastWeekEnd = lastWeekStart + 7 * DAY - 1;
...
export default function App() {
  ...
  const [dateRange, setDateRange] = useState("week"); // week, lastWeek, month, custom
          ...
          {users.filter(u=>u.role==="employee"||u.role==="driver").map(u=>{
            ...
            return(<div key={u.id} className="card" style={{marginBottom:14,borderLeft:`4px solid ${u.color}`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:10}}>
                ...
                <div style={{display:"flex",gap:14,textAlign:"center"}}><div><div style={{fontSize:18,fontWeight:700,color:"#007AFF"}}>{fmtHours(wkMs)}</div><div style={{fontSize:11,color:"#8E8E93"}}>{dateRange==="week"?"semaine":dateRange==="lastWeek"?"semaine passée":dateRange==="month"?"mois":"période"}</div></div><div><div style={{fontSize:18,fontWeight:700,color:u.color}}>{fmtHours(totalMs)}</div><div style={{fontSize:11,color:"#8E8E93"}}>total</div></div></div>
              </div>
